...
const DIFFICULTY_LEVELS = {
  facil: { width: 8, height: 8, mineProbability: 0.10, name: 'Fácil' },
  medio: { width: 12, height: 12, mineProbability: 0.15, name: 'Medio' },
  dificil: { width: 16, height: 16, mineProbability: 0.20, name: 'Difícil' }
};
...
const Buscaminas = () => {
  ...
  const createBoard = () => {
    const { width, height, mineProbability } = getDifficultyConfig();
    const newBoard = Array.from({ length: height }, (_, row) =>
    ...
    if (cell.neighborMines === 0) {
      for (let i = -1; i <= 1; i++) {
        for (let j = -1; j <= 1; j++) {
          ...
          if (
            newRow >= 0 &&
            newRow < getDifficultyConfig().height &&
            newCol >= 0 &&
            newCol < getDifficultyConfig().width &&
            !newBoard[newRow][newCol].isRevealed
          ) {
            revealCell(newRow, newCol);
          }
  ...
  const checkWinCondition = (board) => {
    return board.every((row) =>
      row.every((cell) => cell.isRevealed || (cell.isMine && !cell.isRevealed))
    );
  };
  ...
  return (
    <div className="buscaminas-container">
            ...
            <div
              ...
              onContextMenu={(e) => {
                ...
                if (!currentCell.isRevealed) {
                  ...
                  setFlagCount((prev) => prev + (currentCell.isFlagged ? 1 : -1));
                  setBoard(newBoard);
                }
              }}
            >
      ...
      <PlayerManager
        players={players}
        currentPlayer={currentPlayer}
        onAddPlayer={(name) => setPlayers((prev) => [...prev, { id: Date.now(), name }])}
        onRemovePlayer={(id) => setPlayers((prev) => prev.filter((player) => player.id !== id))}
